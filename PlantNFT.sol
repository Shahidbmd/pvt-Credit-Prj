...
contract PlantNFT is ERC721, Ownable, ReentrancyGuard {
  using Counters for Counters.Counter;
  Counters.Counter private tokenId;
  IERC20 public creditToken;
  string private baseURI;
  uint256 public plantNFTMintFee = 0.000000000000007 ether;
  address private mintFeeReceiver = 0x9f27d8958B96B7Ecf3117184A252DC8d2bb7463D;

  constructor(address _creditToken) ERC721("Plant NFT", "PNFT") {
  ...
  //mint Plant NFT
  function mintPlantNFT(address to) external nonReentrant returns (uint256) {
    _isValidAddress(to);
  ...
  // burn Pnlant NFT
  function burnPlantNFT(uint256 _nftId) external nonReentrant returns (bool) {
    _burn(_nftId);
    return true;
  }

  // Update Minting Fee Only Owner
  function updatePlantNFTFee(uint256 _plantMintFee) external onlyOwner {
