...
contract TreeStakingPool is ERC721Holder, Ownable, ReentrancyGuard {
  uint256 public stakingFeeTreeNFT = 0.007 ether; // Tree NFT StakeFee
  uint256 public minStakingTime = 259200; // min stakingTime is 3days Tree  NFTs
  uint256 public TreeNFTPerSecReward = 0.000000000007 ether; //reward on 1 sec staking
  uint256 public TreeMaintainanceFee = 0.0005 ether; // Tree maintainance Fee
  ...
  IERC721 public immutable TreeNFT;
  ...
  constructor(address _creditToken, address _TreeNFT) {
    creditToken = IERC20(_creditToken);
    TreeNFT = IERC721(_TreeNFT);
  }
  ...
  function stakeNFT(
    ...
    require(msg.value == stakingFeeTreeNFT, "Invalid Staking Fee");
    require(_nftId != 0, "Invalid NFT Id");
    _stakeNFT(_nftId, _agreeToPayFee);
    _transferTreeNFT(msg.sender, address(this), _nftId);
  ...
  function unstakeNFT(uint256 _nftId, uint256 _index) external nonReentrant {
    _unStakeNFT(_index);
    _transferTreeNFT(address(this), msg.sender, _nftId);
  ...
  function claimRewards(uint256 _index) public payable nonReentrant {
    ...
    if (block.timestamp >= maintainanceFeeDeadline + stakedNFT.stakeTime) {
      require(msg.value == TreeMaintainanceFee, "Invalid Fee");
  ...
  function _transferTreeNFT(address from, address to, uint256 _nftId) private {
    TreeNFT.safeTransferFrom(from, to, _nftId);
  }
  ...
  function updateTreeMFee(uint256 _newFee) external onlyOwner {
    _isNotZero(_newFee);
    TreeMaintainanceFee = _newFee;
  }
